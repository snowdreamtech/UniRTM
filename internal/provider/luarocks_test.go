...
func TestLuaRocksProvider_Interface(t *testing.T) {
	var _ Provider = (*LuaRocksProvider)(nil)
}

func TestLuaRocksProvider_Install(t *testing.T) {
	p := NewLuaRocksProvider()
	tmpDir := t.TempDir()
	...
	scriptPath := filepath.Join(binDir, lrName)
...
func TestLuaRocksProvider_Install_NotFound(t *testing.T) {
	t.Setenv("PATH", "")
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "luarocks is required")
}

func TestLuaRocksProvider_ListExecutables(t *testing.T) {
	p := NewLuaRocksProvider()
	tmpDir := t.TempDir()
