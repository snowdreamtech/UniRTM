...
//	backend = "github"
//
//	[tools."github:cli/cli"."platforms.linux-amd64"]
...
type ToolLockEntry struct {
	...
	// omitempty so that tools without platform-specific artifacts (e.g. npm packages)
	// produce clean output.
	Platforms map[string]*PlatformEntry `toml:"platforms,omitempty"`
}
...
func (lf *LockFile) encodeEntry(buf *bytes.Buffer, key string, e *ToolLockEntry) error {
	...
	for _, pk := range platKeys {
		pe := e.Platforms[pk]
		// Write as [tools."key".platforms.linux-amd64] so go-toml can decode
		// it back into the Platforms map field.
		buf.WriteString(fmt.Sprintf("[tools.%q.platforms.%s]\n", key, pk))
		if pe.Checksum != "" {
