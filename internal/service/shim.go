...
func (g *Generator) GenerateShim(ctx context.Context, tool string) error {
	...
	switch runtime.GOOS {
	case "windows":
		if err := g.generateWindowsShim(tool); err != nil {
			return err
		}
	default:
		if err := g.generateUnixShim(tool); err != nil {
			return err
		}
	}
	return nil
...
func (g *Generator) generateUnixShim(tool string) error {
	shimPath := filepath.Join(g.shimsDir, tool)
...
if [ ! -x "${TOOL_BIN}" ]; then
    # Try tool directory directly (some tools don't have bin/ subdir)
...
func (g *Generator) generateWindowsShim(tool string) error {
	envVar := toolVersionEnvVar(tool)
	installRoot := filepath.Join(g.installsDir, tool)
...
if defined %s (
    ...
    echo unirtm: no active version of '%s' found. 1>&2
    echo   Run: unirtm activate %s ^<version^> 1>&2
    exit /b 1
)

set "TOOL_BIN=%s\%%UNIRTM_VERSION%%\bin\%s.exe"
if not exist "%%TOOL_BIN%%" (
    set "TOOL_BIN=%s\%%UNIRTM_VERSION%%\%s.exe"
)
if not exist "%%TOOL_BIN%%" (
    echo unirtm: '%s@%%UNIRTM_VERSION%%' is not installed or binary not found. 1>&2
    exit /b 1
)
...
`,
	...
	cmdPath := filepath.Join(g.shimsDir, tool+".cmd")
	if err := os.WriteFile(cmdPath, []byte(cmdContent), 0644); err != nil {
		return fmt.Errorf("write .cmd shim for %s: %w", tool, err)
	}

	// ── PowerShell (.ps1) shim ───────────────────────────────────────────────
	ps1Content := fmt.Sprintf(`# UniRTM shim for %s
# Generated by UniRTM — do not edit manually

$envVar = $env:%s
if ($null -eq $envVar -or $envVar -eq '') {
    Write-Error "unirtm: no active version of '%s' found."
    Write-Error "  Run: unirtm activate %s <version>"
    exit 1
}

$toolBin = Join-Path "%s" $envVar "bin" "%s.exe"
if (-not (Test-Path $toolBin)) {
    $toolBin = Join-Path "%s" $envVar "%s.exe"
}
if (-not (Test-Path $toolBin)) {
    Write-Error "unirtm: '%s@$envVar' is not installed or binary not found."
    exit 1
}
...
`,
	...
	ps1Path := filepath.Join(g.shimsDir, tool+".ps1")
