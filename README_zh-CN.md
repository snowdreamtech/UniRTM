...
| :--- | :--- | :--- | :--- | :--- |
| **命令执行路径** | 垫片 (Shims) | 垫片(默认) / PATH | **绝对零垫片 (Strict PATH)** | 直接将绝对路径注入环境变量，提供极简的执行路径和透明的 IDE 体验。 |
...
| **垫片开销** | 常规 (Bash 脚本) | 已优化 (Rust 二进制) | **彻底消除 (Zero-Shim)** | 纯 PATH 注入模式，执行即本体，极其适配复杂的 IDE 调试环境。 |
