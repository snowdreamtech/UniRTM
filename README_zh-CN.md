...
| **外部依赖要求** | 极高 (Bash/Curl) | 低 | **绝对零依赖** | 核心插件直接编译入独立二进制，丢进任意极简系统开箱即用。 |
| **垫片开销** | 极高 (Bash 脚本) | 低 (Rust 二进制) | **彻底消除 (Zero-Shim)** | 执行即本体，不仅 100% 无开销，更能避免 IDE 复杂调试环境下的断链问题。 |
| **DevOps 集成** | 困难 | 较高 | **原生级无缝集成** | Go 语言血统，天生匹配云原生基础设施，极其适合企业内部自研平台的二次集成。 |
