...
# Use POSIX-compatible '.' instead of 'source' (bash extension)
if [ -f "${HOME}/.profile" ]; then . "${HOME}/.profile"; fi
if [ -f "${HOME}/.bashrc" ]; then . "${HOME}/.bashrc"; fi

# Extract hook name safely to avoid nested-quote ambiguity in POSIX sh
hook_name="$(basename "$0")"

if command -v unirtm >/dev/null 2>&1; then
    exec unirtm hook run "$hook_name" "$@"
else
    echo "WARNING: UniRTM not found in PATH. Skipping hook." >&2
    exit 0
fi
```
