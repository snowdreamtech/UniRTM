...
## Background
Users running UniRTM inside Windows Git Bash, MSYS2, or Cygwin face issues where native Windows paths (like `C:\...`) are not properly translated, causing path resolution failures when interacting with bash scripts.
...
*   Introduce an environment check for `UNIRTM_CYGDRIVE_PREFIX`.
*   Update `FormatDirForPosix(dir string)` logic:
